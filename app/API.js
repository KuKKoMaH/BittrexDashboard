...
export const bittrex = ( url, params = {}, options, getState ) => {
  ...
  if (!state.bittrexKey || !state.bittrexSecret) return Promise.reject('bittrex keys missing');
...
export const binance = ( url, params, options, getState ) => {
  const state = getState();
  if (!state.binanceKey || !state.binanceSecret) return Promise.reject('binance keys missing');
